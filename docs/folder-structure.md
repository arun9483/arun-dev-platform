...
- `brand.css` — generated brand re-export (gitignored, created by prebuild script)
...
- Generated by prebuild script based on `NEXT_PUBLIC_BRAND` env var
- Re-exports the selected brand CSS from `@arun-dev/tokens`
...
Note: Design tokens live in `packages/tokens/`, not in `apps/web/styles/`. See [docs/design-system.md](../design-system.md) for rationale.
