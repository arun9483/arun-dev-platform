...
- Theme switching logic (FOUC prevention script)

Note: Design tokens come from the published `@arun-dev/tokens` package (maintained in the separate [arun-design-system](https://github.com/arun9483/arun-design-system) monorepo), not from `apps/web/styles/`. See [docs/design-system.md](../design-system.md) for rationale.
...
- Structural tokens (radius, shadows, typography scale)
- Brand presets (color palettes, font families, semantic color mappings)
- Pure CSS — zero runtime dependencies
- White-label compatible (one brand CSS file per identity)
...
- Brand-agnostic (uses CSS variable names, never hardcoded values)
- No business logic
...
# packages from https://github.com/arun9483/arun-design-system
```
