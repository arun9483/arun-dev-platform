...
# @arun-dev/tokens and @arun-dev/ui are published npm packages, maintained in
# the separate arun-design-system monorepo (github.com/arun9483/arun-design-system)
```
...
- Do NOT use hardcoded color/font values — use CSS variables (`var(--token-name)`)
- Do NOT use CSS-in-JS, styled-components, or runtime style generation
- Do NOT import brand CSS files directly in components — only via `globals.css`
- Do NOT put brand-specific styling in shared components — `@arun-dev/ui` is brand-agnostic; changes to it belong in the arun-design-system repo
...
- Use CSS custom properties (`var(--token-name)`) for all visual values
- Use semantic tokens (`--color-bg-primary`) in app/feature code
- Use brand tokens (`--color-brand-500`) only in brand CSS files (maintained in the arun-design-system repo)
- All tokens are defined in the `@arun-dev/tokens` package — never inline in components

---

### White-Labeling

- Brand CSS ships with the `@arun-dev/tokens` package (`@arun-dev/tokens/brands/default`)
- Each brand must satisfy the `BrandSemanticContract` type from `@arun-dev/tokens/createBrand`
- To add a new brand: generate it with `createBrand()` (build-time, in the arun-design-system repo or locally) and swap the brand import in `globals.css`
...
- Dark mode uses `@media (prefers-color-scheme: dark)` + `[data-theme="dark"]`
- FOUC prevention via inline script in `<head>`
- No `next-themes` or similar libraries — implemented with ~100 bytes of inline JS
