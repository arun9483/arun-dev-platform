...
apps/web/
  ...
  lib/               # utilities
  styles/
    brand.css        # generated (gitignored)
    themes/

packages/
  tokens/            # design tokens (pure CSS, white-label compatible)
  ui/                # design system components (brand-agnostic)
...
- Do NOT put brand-specific logic in `packages/ui/` — it must remain brand-agnostic
- Do NOT modify `styles/brand.css` manually — it is generated by the prebuild script
...
- Use brand tokens (`--color-brand-500`) only in `packages/tokens/` brand files
- All tokens are defined in `packages/tokens/` — never inline in components

---

### White-Labeling

- Brand CSS files live in `packages/tokens/src/brands/`
- Each brand file must implement the full brand file contract (see `docs/design-system.md`)
- `apps/web/styles/brand.css` is generated by prebuild script — never edit manually
- To add a new brand: create `brands/<name>.css`, deploy with `NEXT_PUBLIC_BRAND=<name>`
